...
export const BASE_URL = 'http://10.0.2.2:5000'; // Local Android Emulator
...
export const authAPI = {
    ...
    signup: async (userData: any) => {
        try {
            // Calculate a year of birth that matches the age
            const currentYear = new Date().getFullYear();
            const birthYear = currentYear - (userData.age || 25);
            ...
            console.log('Backend signup failed, checking for mock fallback...');

            // For testing/development: allow "signup" to succeed even if backend is offline
            const mockUser = {
    ...
    swipeRight: async (userId: string, comment?: string) => {
        try {
            ...
            if (potentialIndex !== -1) {
                ...
                // Always match for demo/daily-match flow
                const isMatch = true;
